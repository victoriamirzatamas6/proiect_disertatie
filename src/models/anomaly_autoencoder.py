...
def train_autoencoder(X_train: np.ndarray, cfg: dict):
...
def fit_and_score(df_train: pd.DataFrame, df_test: pd.DataFrame, feature_cols: list[str], cfg: dict):
    ...
    df_tr = _filter_normal_part(df_train, float(cfg["normal_cycles_ratio"])) if bool(cfg.get("train_normal_only", False)) else df_train
    X_tr, _ = make_flat_windows(df_tr, feature_cols, window)
    model = train_autoencoder(X_tr, cfg)
    train_scores = reconstruction_error(model, X_tr, batch_size=int(cfg["batch_size"]))
